...
@section('content')
<div class="registro_tn p-3">
    <form method="post" action="{{ route('register.perform') }}" class="container w-25">

        <input type="hidden" name="_token" value="{{ csrf_token() }}" />
        <img class="mb-4" src="{{ asset('assets/img/logo.jpeg') }}" alt="" width="200" height="100">
        ...
        <div class="btn_ctn d-flex flex-column gap-2">
            <button class="w-100 btn btn-lg btn-primary" type="submit">Registrarse</button>
            <button class="w-100 btn btn-lg btn-danger" onclick="window.location.href='{{url('/admin')}}'">Volver
                a home</button>
        </div>
